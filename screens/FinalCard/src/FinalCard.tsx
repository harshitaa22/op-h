...
import FinalCardController, { Props, configJSON } from './FinalCardController'
import Toast from 'react-native-toast-message'


export default class FinalCard extends FinalCardController {
      ...
      <TouchableOpacity
        ...
        onPress={() => {
          if (type == this.timeHorizon) {
            ...
            this.setState({ selectedUpside: index })
          }
        }}
        ...
        {this.state.isLoadingData || this.state.isLoadingPotential ? <ActivityIndicator style={{ flex: 1 }} color={colors.blue} size='large' /> :
          <ScrollView showsVerticalScrollIndicator={false}
            ...
            <View style={styles.container}>
              ...
              <Text style={[styles.text, styles.fontFamilyMedium, { marginTop: scaledSize(10) }]}>{`${configJSON.potential} ${this.state.prevData?.bulish_barish.key == "bullish" ? configJSON.upSide : configJSON.downSide}`}</Text>
              <FlatList
                showsVerticalScrollIndicator={false}
                showsHorizontalScrollIndicator={false}
                scrollEnabled={false}
                data={this.state.prevData?.bulish_barish.key == "bullish" ? this.state.upSides : this.state.downSides}
                keyExtractor={(item, index) => index.toString()}
                renderItem={({ item, index }) =>
