...
import { Props, configJSON } from "./ChannelsCommentsController";
import ChannelsCommentsController from "./ChannelsCommentsController";
//framework
import {
  deviceHeight,
  deviceWidth,
...
export default class ChannelsComments extends ChannelsCommentsController {
  ...
  displayComments = () => {
    return (
      <FlatList
        ...
        data={this.state.comment_All_List}
        ...
        renderItem={({ item, index }: { item: any, index: any }) => {
          ...
          let itemData = this.state.comments_List
          ...
          return (
            ...
            <TouchableOpacity
              ...
              style={[styles.comments_view_row,
              {
                ...
                borderWidth: 1,
                borderColor: heighLight ? colors.blue : this.state.replyId == itemData.id ? colors.blue : colors.white
              }]}>
              <TouchableOpacity
                onPress={() => { this.goToFriendProfilePage(itemData.account.id) }}
                style={{ flex: 1, alignItems: 'center' }}>
                <View style={styles.profileView}>
                  {itemData?.account?.attributes?.share_profile_url ?
                    <Image source={{ uri: itemData?.account?.attributes?.share_profile_url }} style={styles.profile} />
                    : <Image source={profile} style={styles.profile} />}
                  {itemData?.account?.attributes?.opigo_verified == true &&
                ...
                <View style={styles.id_view}>
                  <Text
                    ...
                    {itemData?.account?.attributes?.final_score ? nFormatter(itemData?.account?.attributes?.final_score) : 0}
                  </Text>
                </View>
              </TouchableOpacity>
              <View style={{ flex: 5, justifyContent: 'center' }}>
                <TouchableOpacity onPress={() => { this.goToFriendProfilePage(itemData.account.id) }}>
                  <Text numberOfLines={1} style={[styles.userName, styles.fontFamilyBold,]}>{itemData?.account?.attributes?.first_name} {itemData?.account?.attributes?.last_name}  {" "}</Text>
                </TouchableOpacity>
